...
class Diameter:
    ...
    def __init__(self, OriginHost, OriginRealm,DestRealm ,ProductName, MNC, MCC):
        self.OriginHost = self.string_to_hex(OriginHost)
        self.OriginRealm = self.string_to_hex(OriginRealm)
        self.DestRealm = self.string_to_hex(DestRealm)
    ...
    def Answer_16777252_324(self, packet_vars, avps):
        ...
        prom_diam_eir_event_count.labels(response=EquipmentStatus).inc()
    ...
    def Request_16777291_8388622(self, **kwargs):
        ...
        #MSISDN (Optional)
        if 'msisdn' in kwargs:
            avp += self.generate_vendor_avp(701, 'c0', 10415, self.TBCD_encode(str(kwargs.get('msisdn'))))                                             #Username (MSISDN)

        #GMLC Address
        #avp += self.generate_vendor_avp(2405, 'c0', 10415, self.ip_to_hex('127.0.0.1'))                      #GMLC-Address

        #GMLC Number
        avp += self.generate_vendor_avp(1474, 'c0', 10415, self.TBCD_encode(str(kwargs.get('msisdn'))))      #GMLC-Number
