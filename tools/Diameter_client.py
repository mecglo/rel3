...
recv_ip = yaml_config['mme']['bind_ip']                                                         #IP of this Machine
diameter_host = yaml_config['mme']['OriginHost']                                                        #Diameter Host of this Machine
realm = yaml_config['mme']['OriginRealm']                                          #Diameter Realm of this machine
DestinationHost = yaml_config['mme']['DestinationHost']                                             #Diameter Host of Destination
DestinationRealm = yaml_config['mme']['DestinationRealm']                                       #Diameter Realm of Destination
hostname = yaml_config['mme']['bind_ip']
DestPort = yaml_config['mme']['bind_port']
diameterPeer = yaml_config['mme']['dest_ip']                                                         #IP of this Machine                                  #IP of Remote Diameter Host
mcc = yaml_config['mme']['MCC']                                                                     #Mobile Country Code
mnc = yaml_config['mme']['MNC']                                                                      #Mobile Network Code
transport = yaml_config['mme']['transport']                                                              #Transport Type - TCP or SCTP (SCTP Support is basic)


hostname = diameterPeer 
#hostname = input("Enter IP of Diameter Peer to connect to: ")                                                         #IP of Remote Diameter Host
mcc = yaml_config['mme']['MCC']                                                                     #Mobile Country Code
mnc = yaml_config['mme']['MNC']                                                                      #Mobile Network Code
transport = yaml_config['mme']['transport']                                                              #Transport Type - TCP or SCTP (SCTP Support is basic)

diameter = diameter.Diameter(diameter_host, realm, DestinationRealm ,'mgGMLC-client', str(mcc), str(mnc))
...
try:
    clientsocket.bind(('0.0.0.0', 3868))
    clientsocket.connect((hostname,DestPort))
...
while True:
    ...
    if request == "R":
        ...
        imsi = str(input("IMSI:\t"))
        imei = str(input("IMEI:\t"))
        SendRequest(diameter.Request_16777252_324(imei=imei, imsi=imsi))
